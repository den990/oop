...
const std::regex DOCUMENT_REGEX = std::regex("([\\S]*)?"); // можно ли использовать document string
...
Protocol ParseStringToProtocol(const std::string& stringProtocol) // почитать анониманый namespace
{
...
Port GetDefaultPort(const Protocol& protocol)
{
	switch (protocol)
	{
	case Protocol::HTTP:
...
{
	m_port = port; // добавить проверку на порт
}
...
std::string CHttpUrl::ProtocolToString(Protocol protocol) // переместить в namespace
{
